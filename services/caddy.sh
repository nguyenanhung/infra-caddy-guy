...
restricted_add_whitelist_ips() {
  message INFO "Whitelisting is done on a per-domain basis. Be careful!"

  local sites_path="$CONFIG_DIR/sites"
  local domain domain_file new_ips internal_ips all_ips backup_file

  # Get available domains
  local site_files=$(find "$sites_path" -maxdepth 1 -type f -name "*.caddy" | sed 's|.*/||;s/\.caddy$//')
  [ -z "$site_files" ] && { message ERROR "No domains available"; return 1; }

  # Select domain
  domain=$(echo "$site_files" | fzf --prompt="Select domain: ")
  [ -z "$domain" ] && { message INFO "No domain selected"; return 0; }

  domain_file="$sites_path/$domain.caddy"
  [ ! -f "$domain_file" ] && { message ERROR "Domain file not found: $domain_file"; return 1; }

  confirm_action "Enable restricted Whitelist IPs for ${domain}?" || { message INFO "Operation cancelled!"; return 1; }

  new_ips=$(prompt_with_default "Enter IPs to whitelist (space-separated)" "")
  [ -z "$new_ips" ] && { message ERROR "No IPs provided"; return 1; }
  ...
  awk -v ips="$all_ips" 'BEGIN { inserted = 0 }
    /@blocked_ips not remote_ip/ { print "    @blocked_ips not remote_ip " ips; inserted = 1; next }
    /@acmeChallenge path \/\.well-known\/acme-challenge\// { print; print "    @blocked_ips not remote_ip " ips; print "    respond @blocked_ips \"Access denied\" 403"; inserted = 1; next }
    /^{/ && inserted == 0 { print; print "    @blocked_ips not remote_ip " ips; print "    respond @blocked_ips \"Access denied\" 403"; inserted = 1; next }
    { print }
  ' "$backup_file" > "$domain_file"

  caddy_validate && caddy_reload && message INFO "Whitelist updated for $domain" || {
    mv "$backup_file" "$domain_file"
    message ERROR "Invalid config, restored backup"
    return 1
  }
}

restricted_remove_whitelist_ips() {
  message INFO "Removing whitelisted IPs. Be careful!"

  local sites_path="$CONFIG_DIR/sites"
  local domain domain_file existing_ips remove_ips new_ips backup_file
  ...
  confirm_action "Remove restricted Whitelist IPs for ${domain}?" || { message INFO "Operation cancelled!"; return 1; }

  existing_ips=$(grep -oP '(?<=@blocked_ips not remote_ip ).*' "$domain_file" | tr ' ' '\n' | sort -u)
  [ -z "$existing_ips" ] && { message INFO "No whitelist IPs found"; return 0; }

  remove_ips=$(echo "$existing_ips" | fzf --multi --prompt="Select IPs to remove: ")
  [ -z "$remove_ips" ] && { message INFO "No IPs selected"; return 0; }
  ...
  awk -v ips="$new_ips" 'BEGIN { updated = 0 }
    /@blocked_ips not remote_ip/ { if (ips != "") print "    @blocked_ips not remote_ip " ips; updated = 1; next }
    { print }
  ' "$backup_file" > "$domain_file"
